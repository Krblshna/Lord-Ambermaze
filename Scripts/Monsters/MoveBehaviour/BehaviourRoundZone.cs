...
{
	public class BehaviourRoundZone : MonoBehaviour, IMoveBehaviour
	{
        ...
        private Vector2 CalcInitMoveVector()
        {
            ...
            if (AccessibleMove(Vector2Int.up))
            {
                if (AccessibleMove(Vector2Int.right))
                {
                    _moveDirection = _clockwise ? Vector2.up : Vector2.right;
                }
                else
                {
                    _moveDirection = _clockwise ? Vector2.left : Vector2.up;
                }
            }
            else
            {
                if (AccessibleMove(Vector2Int.right))
                {
                    _moveDirection = _clockwise ? Vector2.right : Vector2.down;
                }
                else
                {
                    _moveDirection = _clockwise ? Vector2.down : Vector2.left;
                }
            }
        ...
        private bool AccessibleMove(Vector2Int moveVector)
        {
